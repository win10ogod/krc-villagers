...
**預設 Shift＋右鍵開啟村民管理，普通右鍵仍是原版交易；管理快捷鍵可在遊戲內調整。**
...
以下是 **1.2.0 的測試組合**。自動更新發布的版本，請以該 Release 的依賴下載表與隨附 `dependencies.lock.json` 為準；每次更新都需重新通過編譯及遊戲測試。
...
在 **選項 → 控制 → 按鍵設定 → 假面騎士村民 → 開啟村民管理** 修改快捷鍵。可使用鍵盤、滑鼠按鈕或 Shift／Ctrl／Alt 組合；點選按鍵欄後輸入新組合，按 Esc 可取消綁定，「重設」恢復 Shift＋右鍵。設定會隨遊戲選項保存，重新開啟遊戲後保留。可參考[按鍵設定畫面](evidence/18-rebound-key-setting.png)。

準星對準互動距離內的村民再按快捷鍵。改成單鍵後不必蹲下；改成其他按鍵後，原本的 Shift＋右鍵回到原版互動。若綁定普通右鍵，對村民按右鍵就會優先開啟管理；想保留交易操作，可改用其他按鍵或修飾鍵組合。各玩家可分別設定，與原版「潛行」按鍵獨立；多人遊戲請將伺服器和客戶端一併更新至 1.2.0。

1. 對成年村民按管理快捷鍵（預設 **Shift＋右鍵**），花費 **8 顆綠寶石**招募。創造模式免收費，招募人數不設上限。
...
遊戲測試必須全部通過；Gradle 會比對測試程式中的項目數與伺服器的成功結果，缺少測試結果也會使工作流失敗。目前共 40 項 GameTest。工作流不執行圖形介面測試，客戶端驗證使用下方 `runClient -PuiSmoke`。
